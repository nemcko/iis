...
import { DlgAddThemeComponent } from './dlgtheme.component';
...
/**
 * Component: Stretnutie s team-leaderom 1:1
 */
@Component({
  templateUrl: './ep-meeting.component.html',
  styleUrls: ['../eportal.css']
})
export class EpMeetingComponent implements OnInit {
  public idl: string = '';
  public id: string;
  public data: any = {};
      ...
      (param) => {
        ...
        if (param['id'] && param['id'] === 'new') {
          ...
          if (param['id']) {
            this.id = param['id'];
            forkJoin(
              this.eportalSvc.getMeeting(this.id),
            ).subscribe(
              data => {
                this.data = data[0];
                this.loader.showSpinner(false);
  ...
  public get hasThemes(): boolean {
    return this.data && this.data.themes && this.data.themes.length;
  }
  ...
  public saveData(publish: boolean = false): void {
    ...
    this.eportalSvc.setMeeting(this.data).subscribe(
      () => {
        this.loader.showSpinner(false);
